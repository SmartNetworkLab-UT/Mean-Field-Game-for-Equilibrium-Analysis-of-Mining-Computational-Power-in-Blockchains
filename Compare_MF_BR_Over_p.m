...
p_Vector = 0:0.5:4;
p_Range = 7;
MF_SWF_Vector = zeros(size(p_Vector));
BR_SWF_Vector = zeros(size(p_Vector));
N = 1000;
...
for simulationRun = 1:length(p_Vector)
p_base = p_Vector(simulationRun);
disp(['BR and MF, p=', num2str(p_base), ':', num2str(p_base+p_Range)]);
%% Creating agents
...
for i = 1:N
    ...
    p = p_base + p_Range*rand; %2 + 7*i/N; %2 + 7*rand; %uniform(2,9)
...
%% Ploting the results
figure
plot(p_Vector, MF_SWF_Vector, 'o-', 'DisplayName', 'MF', 'LineWidth',2);
hold on
plot(p_Vector, BR_SWF_Vector, 'o-', 'DisplayName', 'BR', 'LineWidth',2);
hold off

xticks(p_Vector)
TickLabelCell = cell(1, length(p_Vector));
for i = 1:length(p_Vector)
    TickLabelCell{i} = ['p=', num2str(p_Vector(i)),':', num2str(p_Vector(i)+p_Range)];
end
xticklabels(TickLabelCell)
ylabel('Social Welfare');
title('Social Welfare for Different p parameter')
legend('FontSize', 14);
toc;